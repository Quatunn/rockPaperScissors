var dndSupported;    // Verdadeiro se o  drag and drop for suportado
...
function handleDragStart(e) {
    ...
    e.dataTransfer.setDragImage(getRPSImg(this), 120, 120); // Escolhe a imagem arrastada
...
function handleDragEnd(e) {
    ...
    // reenicia o ciclo dos elementos
    draggingElement.className = undefined;
    draggingElement = undefined;

    // reenicia todos os elementos
...
function handleDragOver(e) {
    ...
    return false;   // Alguns navegadores poderão precisar disto para evitar a ação pré-definida
}
...
function handleDrop(e) {
    if(e.stopPropegation) e.stopPropagation(); // Impede alguns navegadores de redirecionar
    if(e.preventDefault) e.preventDefault();
    if(this.id === draggingElement.id) return;
    else isWinner(this, draggingElement);
}

// Funções com utilidade
...
// Suporte vitalicio para a aplicação
...
function init() {
    if((dndSupported = detectDragAndDrop())) {
        ...
        statusMessage('This browser does not support Drag and Drop');  // mensagem pop up que indica que o navegador nao suporta o DnD
    }
}
